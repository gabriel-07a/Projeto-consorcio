...
public class GrupoAbertoState implements GrupoState{
    ...
    public void validarNovoParticipante(GrupoConsorcio grupoConsorcio, Usuario usuario) {
        ...
        if(totalCotasAtual >= grupoConsorcio.getVagasMaximas()) throw new RegraDeNegocioException("Erro: Esse grupo de consórcio já está lotado");
    ...
    public void validarSorteio() {
        throw new RegraDeNegocioException("Erro: Esse grupo ainda não está em andamento!");
    }
    ...
    public void encerrarConsorcio() {
        throw new RegraDeNegocioException("Erro: Esse grupo não está em andamento!");
    }

    @Override
    public void validaExclusaoDeConsorcio(GrupoConsorcio grupoConsorcio, String enderecoContrato) {
        if(!grupoConsorcio.getCotas().isEmpty()) throw new RegraDeNegocioException("Erro: Um grupo com participantes não pode ser excluído!");
        if(!grupoConsorcio.getEnderecoContrato().equalsIgnoreCase(enderecoContrato)) throw new RegraDeNegocioException("Erro: O endereço do Smart Contract do grupo está errado!");
